...
/// <summary>
/// This is super confusing: OpenAI calls it "completions" and Claue "messages".
/// Which is why I'm calling this class "Completions" and not "Messages".
/// </summary>

public class Completions {
   private const           string Endpoint          = "messages";
   private static readonly string Full_Endpoint_URL = ClaudeAI.HTTP_Client.Get_full_URL_for_endpoint(Endpoint);
   ...
   public static async Task<Response?> POST_Async(Request request) {
      ...
      File.WriteAllText("_request_claude.json", request_content_json);
      ...
      try {
         // Make the request
         HttpResponseMessage responseMessage = await ClaudeAI.HTTP_Client.Gimme().PostAsync(Full_Endpoint_URL, content).ConfigureAwait(false);
